...
- (void)viewDidLoad {
    ...
    self.ttableView.frame = CGRectMake(275, 155, 100, 25);
    
    _ttableView.delegate = self;
    _ttableView.dataSource = self;
    
    self.ttButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [self.view addSubview:_ttButton];
    self.ttButton.frame = CGRectMake(375, 155, 25, 25);
    [self.ttButton setImage:[UIImage imageNamed:@"close1.png"] forState:UIControlStateNormal];
    [self.ttButton addTarget:self action:@selector(pressOpen:) forControlEvents:UIControlEventTouchDown];
    ...
    _localImageView.frame = CGRectMake(275, 120, 25, 25);
    
    _localLabel = [[UILabel alloc] init];
    _localLabel.text = @"陕西省，西安市";
    ...
    _localLabel.frame = CGRectMake(304, 120, 100, 25);
...
- (void)pressOpen:(UIButton *)btn{
    ...
    if(_openSelect == NO){
        self.ttableView.frame = CGRectMake(275, 155, 100, 100);
        _openSelect = YES;
